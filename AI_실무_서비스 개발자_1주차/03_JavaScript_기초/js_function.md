...
함수는 어떤 `동작`을 수행하기 위한 코드를 모아놓은 것이다. 따라서 함수의 이름은 대개 동사이다. 함수가 어떤 동작을 하는지 설명할 수 있어야 하기 때문에 코드를 읽는 사람은 함수 이름만 보고도 어떤 기능을 하는지 힌트를 얻을 수 있도록 가독성이 좋게 작성하자
